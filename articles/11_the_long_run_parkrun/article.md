...
The later recovery is also informative. Raw times do not return to the personal-best cluster, yet age-grade performance improves toward the end. Fitness can therefore develop even when absolute times do not recreate an earlier peak exactly.
...
The apparent seasonal pattern is therefore partly a form pattern. The exceptional 2021 block pulls late-autumn and early-winter averages down, while slower phases in other years lift different months. Month provides context; the multi-year trajectory explains why the same month does not produce the same performance.
...
This is not a verdict on what a person can achieve with changed training or conditions. The model extends the statistical structure already present in the record. It says that without new information, the 2021 peak should be treated as exceptional rather than assumed to be the current baseline.
...
Read together, the record argues for patient interpretation. Fast months matter, but so do weather, attendance patterns and the level from which they were achieved. Progress is best understood as a shifting range with occasional breakthroughs, not as a straight line that every future run is obliged to follow.

## Progress is a range with occasional breakthroughs

The five views give the personal story its shape. The distribution defines normal performance. The time series identifies the concentrated breakthrough. Age grade confirms its quality and shows later improvement in context. Monthly averages reveal seasonality entangled with form. The forecast quantifies how uncertain a return to the fastest tail remains.
